...
# Разблокировка функции ловушек
TRAP_UNLOCK_LOCATION = 4

# Животные + лут по локациям (еда.txt)
TRAP_LOOT_TABLE: Dict[int, List[Dict[str, Any]]] = {
    ...
    2: [
        ...
        {"name": "Камышовый козёл", "chance": 40, "loot": {"Сырое мясо": 2, "Кожа": 1, "Кости": 1}},
    ],
    3: [
    ...
    4: [
        {"name": "Дикий кабан", "chance": 50, "loot": {"Сырое мясо": 4, "Кожа": 2, "Кости": 2}},
        {"name": "Молодой олень", "chance": 50, "loot": {"Сырое мясо": 5, "Кожа": 3, "Кости": 2, "Мех": 3}},
    ],
    5: [
    ...
    6: [
        ...
        {"name": "Горный баран", "chance": 40, "loot": {"Сырое мясо": 3, "Мех": 2, "Кости": 2}},
    ],
    7: [
...
TRAP_CHANCE_BY_LOCATION: Dict[int, int] = {i: 60 for i in range(1, 8)}  # успех после «не сломалась»


def traps_unlocked(game_state) -> bool:
    """Ловушки доступны, если игрок дошёл до L4 или уже ставил ловушку."""
    flags = getattr(game_state, "story_flags", {}) or {}
    if flags.get("traps_unlocked"):
        return True
    loc = int(getattr(game_state, "location_index", 0) or 0)
    # location_index 0-based → L4 = index 3; также current location id
    if loc >= 3:
        return True
    # по имени
...
def roll_trap_roll(game_state, location_id: int) -> Optional[Dict]:
    """
    Утренняя проверка одной ловушки.
    40% — ломка; 60% — успех + животное/лут из таблицы локации.
    """
    location_id = int(location_id)
    traps = getattr(game_state, "traps", {}) or {}
...
def process_trap_rollover(game_state) -> List[Dict]:
    """Все активные ловушки после сна. Возвращает список событий для лога."""
    traps = getattr(game_state, "traps", {}) or {}
    results = []
    for loc_id in list(traps.keys()):
        trap = traps[loc_id]
        if not trap.get("is_active") and not trap.get("is_broken"):
            continue
        if not trap.get("is_active") and trap.get("is_broken"):
            continue
