...
# location_id 1..7 → список {item, chance}
...
def _roll_table(table: List[Dict[str, Any]]) -> List[str]:
    """Один предмет из взвешенной таблицы (сумма chance ≈ 100)."""
    if not table:
        return []
...
def roll_find(location_id: int) -> List[str]:
    """Основной лут + бонусные броски. Список имён предметов (могут повторяться)."""
...
def apply_finds_to_inventory(game, found: List[str]) -> str:
    """Добавить находки в инвентарь, вернуть текст для лога."""
    if not found:
